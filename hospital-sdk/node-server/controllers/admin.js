...
const createPatient = async (req, res) => {
  // User role from the request header is validated
  // const userRole = req.headers.role;
  let { username, password, address, phoneNumber, bloodGroup, hospitalId, age } = req.body;

  // const isValidate = await validateRole([ROLE_ADMIN], userRole, res);
  // if (isValidate) return res.status(401).json({ message: "Unauthorized Role" });

  req.body.userId = username;
  req.body.role = ROLE_PATIENT;
  ...
  console.log("After network call", response)
  if (response.error) {
    return res.status(400).send(response.error);
  }


  const isPatientExist = await UserDetails.findOne({ username: username });
  ...
  const { accessToken, refreshToken } = generateTokens(username, ROLE_PATIENT);
...
const getAllPatients = async (req, res) => {
  try {
    ...
    const response = await network.invoke(
      networkObj,
      true,
      capitalize(userRole) + "Contract:queryAllPrescriptions",
      userRole === ROLE_DOCTOR ? req.headers.username : [""]
    );
    const parsedResponse = await JSON.parse(response);
    res.status(200).send(parsedResponse);
