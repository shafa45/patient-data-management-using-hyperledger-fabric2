...
import {
  ...
  PatientPrescritionDetailsUpdateByDoctor
} from "@/types/patient";
...
type patientDetail = {
  patientprescriptionDetailDetail: PatientPrescritionDetailsUpdateByDoctor;
  closeModal: () => void;
};
const UpdatePatientDetailsByDoctor: React.FC<patientDetail> = ({
    patientprescriptionDetailDetail,
  closeModal,
}) => {
  const dispatch = useAppDispatch();
  const [isSubmitting, setIsSubmitting] = useState(false);

  const UpdateDetails = async (patientprescriptionDetailDetail: PatientPrescritionDetailsUpdateByDoctor) => {
    ...
    //   );
      await axiosInstance.patch(
        `${API_BASE_URL}/doctors/patients/${patientprescriptionDetailDetail.prescriptionId}/details/medical`,
        patientprescriptionDetailDetail
      );

    //   dispatch(getPatientsUnderDoctor());
      dispatch(getPrescritpionsUnderDoctor());
      closeModal();
      toast.dismiss();
    ...
    <div className="flex  justify-center mt-8">
      <div className="bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md  xl:p-0 dark:bg-gray-800 dark:border-gray-700">
        <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
          ...
          <Formik initialValues={patientprescriptionDetailDetail} onSubmit={UpdateDetails}>
            {() => (
