...
import { PrescriptionHistory } from "@/types/patient";
import { capitalize, convertTimestamp } from "@/utils/convertTime";
import DocViewer, { DocViewerRenderers } from "@cyntler/react-doc-viewer";

const PrescriptionHistoryRecords: React.FC<{ prescriptionId: string }> = ({
  prescriptionId,
}) => {
  ...
  const HistoryOfPrescription = () => {
    ...
    if (prescriptionHistory.length === 0)
      return <div> No History For {prescriptionId}</div>;

      console.log("prescriptionHistory", prescriptionHistory)
      prescriptionHistory.splice(-1,1)
    return (
      <div>
        <table className="table-auto min-w-950 overflow-auto">
          ...
          <tbody>
            {prescriptionHistory.map((data: PrescriptionHistory, index: number) => {
  ...
  return (
    <div className="header min-w-fit rounded-3xl  px-3 py-1">
      <div className="p-3">
        <i className="fas fa-book mr-1"></i> Prescription History
        {/* <span className="font-semibold">{prescriptionId}</span> */}
      </div>
      <div className="p-3">
...
export default PrescriptionHistoryRecords;
